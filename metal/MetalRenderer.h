...
// PORTED: AppKit→UIKit (Audit Sec 4.3)
...
struct PipelineSelectorMTL
{
    ...
    PipelineSelectorMTL() { memset(this, 0, sizeof(*this)); }
    ...
    PipelineSelectorMTL(const PipelineSelectorMTL& other) { memcpy(this, &other, sizeof(other)); }
    PipelineSelectorMTL& operator=(const PipelineSelectorMTL& other) { memcpy(this, &other, sizeof(other)); return *this; }
    bool operator==(const PipelineSelectorMTL& other) const { return BitEqual(*this, other); }
};
...
public:
    ...
    ~GSScopedDebugGroupMTL() { [m_buffer popDebugGroup]; }
};
...
public:
    ...
    enum class UsePresentDrawable : u8 { Never, Always, IfVsync };
    enum class LoadAction { DontCare, DontCareIfFull, Load };

    class UsageTracker
    {
        struct UsageEntry { u64 drawno; size_t pos; };
    ...
    struct VSSelector
    {
        union
        {
            struct { bool iip:1; bool fst:1; bool point_size:1; GSShader::VSExpand expand:3; };
            u8 key;
        };
        VSSelector(): key(0) {}
    };
    ...
    // Permanent resources
    std::shared_ptr<std::pair<std::mutex, MetalRenderer*>> m_backref;
    GSMTLDevice m_dev;
    ...
    struct MainRenderEncoder
    {
        ...
        struct Has
        {
            bool cb_vs:1; bool cb_ps:1; bool scissor:1; bool blend_color:1;
            bool pipeline_sel:1; bool sampler:1; bool rt1_depth:1;
        } has = {};
    ...
    void EndRenderPass();
    void BeginRenderPass(NSString* name, GSTexture* color, MTLLoadAction color_load,
        GSTexture* depth, MTLLoadAction depth_load, GSTexture* stencil = nullptr,
        MTLLoadAction stencil_load = MTLLoadActionDontCare, bool rt1 = false);
    void FrameCompleted();
    ...
    void DoInterlace(GSTexture* sTex, const GSVector4& sRect, GSTexture* dTex,
        const GSVector4& dRect, ShaderInterlace shader, bool linear, const InterlaceConstantBuffer& cb) override;
    ...
    void DoStretchRect(GSTexture* sTex, const GSVector4& sRect, GSTexture* dTex,
        const GSVector4& dRect, id<MTLRenderPipelineState> pipeline, bool linear,
        LoadAction load_action, const void* frag_uniform, size_t frag_uniform_len);
    ...
    void MRESetHWPipelineState(GSHWDrawConfig::VSSelector vs, GSHWDrawConfig::PSSelector ps,
        GSHWDrawConfig::BlendState blend, GSHWDrawConfig::ColorMaskSelector cms);
    ...
    void SendHWDraw(GSHWDrawConfig& config, id<MTLRenderCommandEncoder> enc,
        id<MTLBuffer> buffer, size_t off, bool one_barrier, bool full_barrier);
...
protected:
    virtual void DoStretchRect(GSTexture* sTex, const GSVector4& sRect, GSTexture* dTex,
        const GSVector4& dRect, GSHWDrawConfig::ColorMaskSelector cms, ShaderConvert shader, bool linear) override;
};
...
static constexpr bool IsCommandBufferCompleted(MTLCommandBufferStatus status)
{
    switch (status)
    {
        ...
        case MTLCommandBufferStatusScheduled: return false;
        case MTLCommandBufferStatusCompleted:
        case MTLCommandBufferStatusError: return true;
    }
}
